...
import com.zpj.widget.switcher.R;
...
public abstract class BaseSwitcher extends View {
    ...
    public void setEnabled(boolean enabled) {
        ...
        setCurrentColor(enabled ? (isChecked ? onColor :offColor) : Color.LTGRAY);
        postInvalidate();
    }
    ...
    protected void initAttributes(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        setCurrentColor(isEnabled() ? (isChecked ? onColor :offColor) : Color.LTGRAY);
    ...
    protected void generateAnimates(Set<Animator> animatorSet) {
        ...
        int toColor =  isEnabled() ? (isChecked ? offColor : onColor) : Color.LTGRAY;
    ...
    public void setChecked(boolean checked, boolean withAnimation) {
        if (isEnabled() && this.isChecked != checked) {
            if (withAnimation) {
                ...
                if (!checked) {
                    setCurrentColor(isEnabled() ? offColor : Color.LTGRAY);
                    setIconProgress(1f);
                } else {
                    setCurrentColor(isEnabled() ? onColor : Color.LTGRAY);
                    setIconProgress(0f);
                }
            }
        }
