...
//        }
        Log.d("toObservable", "key=" + key);
        return mSubject.ofType(String.class)
                .filter(new Predicate<String>() {
                    @Override
                    public boolean test(@NonNull String s) throws Exception {
                        Log.d("toObservable", "test key=" + key + " s=" + s);
...
//                                && TextUtils.equals(this.key, key);
    ...
    public <S, T> Observable<RxSubscriber.PairMessage> toObservable(final String key, final Class<S> type1, final Class<T> type2) {
        return mSubject.ofType(RxSubscriber.PairMessage.class)
                .filter(msg -> {
                    Object s = msg.getS();
                    Object t = msg.getT();
                    boolean isSameKey = TextUtils.equals(msg.getKey(), key);
                    if (isSameKey) {
                        if (type1 != null && type2 != null) {
                            return type1.isInstance(s) && type2.isInstance(t);
                        }
                    }
                    return isSameKey;
                });
